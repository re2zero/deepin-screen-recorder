...
def make_config_parser():
    ...
    # deepin-screen-recorder may contain both DTK-style camelCase keys
    # and legacy lowercase keys (for example isBlur/isblur).  Keep key
    # case and tolerate duplicates so tests can update unrelated options
    # on machines with existing user configuration.
...
def wait_video(directory, suffixes, marker, min_mtime=0, timeout=30):
    deadline = time.time() + timeout
    ...
    while time.time() < deadline:
        ...
        for path in root.iterdir() if root.exists() else []:
            if (
                ...
                and not ffmpeg_running
            ):
                mark(marker, str(path))
                return
        time.sleep(0.5)
    suffix_text = ",".join(sorted(suffixes))
    raise AssertionError(f"no video ({suffix_text}) generated in {directory}")


def wait_mp4(directory, marker):
    wait_video(directory, {".mp4"}, marker)
...
def main():
    ...
    if command == "root":
        ...
        expected_format = sys.argv[5] if len(sys.argv) > 5 and sys.argv[5] != "any" else None
        wait_image(sys.argv[2], set(sys.argv[3].split(",")), sys.argv[4], expected_format)
    ...
    elif command == "wait-video":
        min_mtime = float(sys.argv[5]) if len(sys.argv) > 5 else 0
        wait_video(sys.argv[2], set(sys.argv[3].split(",")), sys.argv[4], min_mtime)
